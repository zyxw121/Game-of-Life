...
/** Configurations are represented as a text file. The first uncommented line is of the form
  * "x = [INT], y= [INT], rule = B[INT]/S[INT] " which gives the dimensions of
  * the starting pattern, and  optionally, the ruleset.  The second line is an RLE string
  * representing the state where:
  *   o :   Live cell
  *   b :   Dead cell
  *   $ :   Line break
  * Improperly formatted configuration files may result in errors.
  */
  ...
  class Worker(y : Int,  height : Int) {
    ...
    // One worker calls this every iteration.
    protected def manage = { }

/** First, wait at the barrier for all the workers to be ready. Then repeatedly
  * copy and wait for the others, and update the board and wait for the others.
  */
  ...
  // The Manager is in charge of updating the display and keeping track of the generation
  ...
  // First arg is configuration source file, second is number of generations to run for
